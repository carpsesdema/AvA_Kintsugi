...
class ActionService:
    ...
    def handle_new_project(self):
        ...
        if not all([project_manager, rag_manager, app_state_service, project_indexer]): return
        ...
        project_indexer.clear_index() # Clear the index for the new project.
    ...
    def handle_load_project(self):
        ...
        if path:
            ...
            if project_path_str:
                project_path = Path(project_path_str)
                # THIS IS THE FIX: Trigger the background scan of the project.
                asyncio.create_task(project_indexer.build_index(project_path))
                asyncio.create_task(rag_manager.switch_project_context(project_path))

                self.log("info", "Project loaded. Indexing started in background.")
                app_state_service.set_app_state(AppState.MODIFY, project_manager.active_project_name)
