...
# V11: Makes task cancellation an async function for proper awaiting.
...
class Application:
    ...
    all major UI components and services, with graceful task management.
    """
    ...
    def on_user_request(self, prompt: str, history: list):
        self.workflow_monitor.scene.setup_layout()  # Reset monitor for new run
    ...
    async def cancel_all_tasks(self):
        """
        Cancels all running background tasks and waits for them to acknowledge
        the cancellation. This is now an async function.
        """
        ...
        # The 'return_exceptions=True' ensures that we don't crash if a task
        # has a weird error during cancellation.
