...
class ServiceManager:
    ...
    def launch_background_servers(self):
        ...
        if self.llm_server_process is None or self.llm_server_process.poll() is not None:
            ...
            try:
                ...
                error_details = f"Failed to launch LLM server: {e}\n{traceback.format_exc()}"
                self.log_to_event_bus("error", error_details)
        else:
            self.log_to_event_bus("info", "LLM Server already running or launch attempt in progress.")
        ...
        if self.rag_server_process is None or self.rag_server_process.poll() is not None:
            ...
            try:
                ...
                error_details = f"Failed to launch RAG server: {e}\n{traceback.format_exc()}"
                self.log_to_event_bus("error", error_details)
        else:
            self.log_to_event_bus("info", "RAG Server already running or launch attempt in progress.")
    ...
    def terminate_background_servers(self):
        ...
        servers = {"LLM": self.llm_server_process, "RAG": self.rag_server_process}
