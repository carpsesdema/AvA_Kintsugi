...
# V8: Enforces consistent event signature for prepare_for_generation.
...
class ArchitectService:
    ...
    async def _handle_modification(self, prompt: str, existing_files: dict, user_prompt_summary: str) -> bool:
        ...
        # --- THE FIX: Always emit two arguments. For modifications, the path is None. ---
        self.event_bus.emit("prepare_for_generation", [f['filename'] for f in file_plan], None)
        await asyncio.sleep(0.1)
    ...
    async def _generate_and_apply_patch(self, filename: str, purpose: str, original_code: str,
        ...
        if success:
            ...
            if filename in updated_files: self.event_bus.emit("code_patched", filename, updated_files[filename],
                                                              cleaned_patch)
            return True
        else:
    ...
    def _clean_code_output(self, code: str, is_diff: bool = False) -> str:
        ...
        if code.startswith(start_marker):
            ...
            code = code[3:].lstrip()
            if is_diff and code.lstrip().startswith('diff'): code = code.lstrip()[4:].lstrip()
